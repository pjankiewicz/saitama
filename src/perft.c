...
const PERFT_T pertfsuite[] = {
//        {"rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1",{20,400,8902,197281,4865609,119060324}},
//        {"r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq - 0 1",{48,2039,97862,4085603,193690690,0}},
...
void TestOnePerft(U64 perft_result, U64 perft_expected, const S_BOARD *pos, int perf_test, int *n_tests, int *n_passed,
                  int *n_failed);

U64 Perft(S_BOARD *pos, int depth) {
    ...
    GenerateAllMoves(pos, moveslist);
    for (int i = 0; i < moveslist->count; i++) {
        legal = MakeMove(pos, moveslist->moves[i].move);
        if (legal) {
            for (int his=0; his < pos->hisPly; his++) {
                printf("%s ", PrMoveDet(pos->history[his].move));
            }
            PrintBoard(pos);
            printf("\n");
...
void TestPerf() {
    ...
    int n_failed = 0;
    U64 perft_result = 0;
    U64 perft_expected = 0;
    S_BOARD pos[1];
    for (int perf_test = 0; perf_test < 126; perf_test++) {
        TestOnePerft(perft_result, perft_expected, pos, perf_test, &n_tests, &n_passed, &n_failed);
    }
...
void TestOnePerft(U64 perft_result, U64 perft_expected, const S_BOARD *pos, int perf_test, int *n_tests, int *n_passed,
                  int *n_failed) {
    ParseFen(pertfsuite[perf_test].fen, pos);
    PrintBoard(pos);
    for (int depth = 0; depth < 2; depth++) {
        perft_result = Perft(pos, depth + 1);
        perft_expected = pertfsuite[perf_test].depths[depth];
        if (perft_expected == 0) {
            break;
        }
        printf("Test %d Depth=%d Result=%llu Expected=%llu [%s]\n",
               perf_test + 1, depth + 1, perft_result, perft_expected,
               perft_expected == perft_result ? "PASS" : "FAIL!!!!!!!!!!!!!!!");
        (*n_tests) += 1;
        if (perft_expected == perft_result) {
            (*n_passed) += 1;
        } else {
            (*n_failed) += 1;
            break;
